...
NX_CONSTEXPR CmifRequest cmifMakeRequest(void* base, CmifRequestFormat fmt) {
    ...
    u32 actual_size = 16;
    if (fmt.object_id) actual_size += sizeof(CmifDomainInHeader) + fmt.num_objects * sizeof(u32);
    ...
    CmifRequest req = {};
    req.hipc = hipcMakeRequestInline(
        base, .type = fmt.context ? CmifCommandType_RequestWithContext : CmifCommandType_Request,
        .num_send_statics = fmt.num_in_auto_buffers + fmt.num_in_pointers,
        .num_send_buffers = fmt.num_in_auto_buffers + fmt.num_in_buffers,
        .num_recv_buffers = fmt.num_out_auto_buffers + fmt.num_out_buffers, .num_exch_buffers = fmt.num_inout_buffers,
        .num_data_words = num_data_words, .num_recv_statics = out_pointer_size_table_size + fmt.num_out_fixed_pointers,
        .send_pid = fmt.send_pid, .num_copy_handles = fmt.num_handles, .num_move_handles = 0, );
...
NX_CONSTEXPR void* cmifMakeControlRequest(void* base, u32 request_id, u32 size) {
    ...
    HipcRequest hipc =
        hipcMakeRequestInline(base, .type = CmifCommandType_Control, .num_data_words = (actual_size + 3) / 4, );
...
NX_CONSTEXPR void cmifMakeCloseRequest(void* base, u32 object_id) {
    if (object_id) {
        HipcRequest hipc = hipcMakeRequestInline(base, .type = CmifCommandType_Request,
        ...
        *domain_hdr = (CmifDomainInHeader){
        ...
        hipcMakeRequestInline(base, .type = CmifCommandType_Close, );
    }
}
...
NX_CONSTEXPR void cmifRequestObject(CmifRequest* req, u32 object_id) { *req->objects++ = object_id; }

NX_CONSTEXPR void cmifRequestHandle(CmifRequest* req, Handle handle) { *req->hipc.copy_handles++ = handle; }
...
NX_CONSTEXPR Result cmifParseResponse(CmifResponse* res, void* base, bool is_domain, u32 size) {
    ...
    if (R_FAILED(hdr->result)) return hdr->result;
...
NX_CONSTEXPR u32 cmifResponseGetObject(CmifResponse* res) { return *res->objects++; }

NX_CONSTEXPR Handle cmifResponseGetCopyHandle(CmifResponse* res) { return *res->copy_handles++; }

NX_CONSTEXPR Handle cmifResponseGetMoveHandle(CmifResponse* res) { return *res->move_handles++; }
...
NX_INLINE Result cmifConvertCurrentObjectToDomain(Handle h, u32* out_object_id) {
    ...
    if (R_SUCCEEDED(rc)) {
        ...
        if (R_SUCCEEDED(rc) && out_object_id) *out_object_id = *(u32*)resp.data;
    }
    return rc;
}
...
NX_INLINE Result cmifCopyFromCurrentDomain(Handle h, u32 object_id, Handle* out_h) {
    ...
    *(u32*)raw = object_id;
    ...
    if (R_SUCCEEDED(rc)) {
        ...
        if (R_SUCCEEDED(rc) && out_h) *out_h = resp.move_handles[0];
    }
    return rc;
}
...
NX_INLINE Result cmifQueryPointerBufferSize(Handle h, u16* out_size) {
    ...
    if (R_SUCCEEDED(rc)) {
        ...
        if (R_SUCCEEDED(rc) && out_size) *out_size = *(u16*)resp.data;
    }
    return rc;
}
