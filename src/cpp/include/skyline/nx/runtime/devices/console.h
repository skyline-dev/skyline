...
typedef enum {
    ...
    debugDevice_SVC,   ///< Outputs stderr debug statements using svcOutputDebugString, which can then be captured by
 ...
 * This should only be used when using a single console or without changing the console that is returned, otherwise use
 * consoleInit().
 ...
 * @param console A pointer to the console data to initialize (if it's NULL, the default console will be used).
 * @remark This function should be called periodically. Failure to call this function will result in lack of screen
 * updating.
 */
