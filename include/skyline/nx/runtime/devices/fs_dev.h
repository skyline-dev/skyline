...
NX_CONSTEXPR FsDirectoryEntry* fsdevDirGetEntries(fsdev_dir_t* dir) { return (FsDirectoryEntry*)(void*)(dir + 1); }
...
Result fsdevMountSystemSaveData(const char* name, FsSaveDataSpaceId save_data_space_id, u64 system_save_data_id,
                                AccountUid uid);

/// Mounts the input fs with the specified device name. fsdev will handle closing the fs when required, including when
/// fsdevMountDevice() fails. Returns -1 when any errors occur.
int fsdevMountDevice(const char* name, FsFileSystem fs);

/// Unmounts the specified device.
int fsdevUnmountDevice(const char* name);

/// Uses fsFsCommit() with the specified device. This must be used after any savedata-write operations(not just
/// file-write). This should be used after each file-close where file-writing was done. This is not used automatically
/// at device unmount.
...
/// Writes the FS-path to outpath (which has buffer size FS_MAX_PATH), for the input path (as used in stdio). The
/// FsFileSystem is also written to device when not NULL.
