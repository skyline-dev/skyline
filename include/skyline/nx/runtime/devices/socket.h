...
typedef enum {
    ...
    BsdServiceType_Auto =
        BsdServiceType_User | BsdServiceType_System,  ///< Tries to use bsd:s first, and if that fails uses bsd:u
                                                      ///< (official software behavior).
} BsdServiceType;
...
typedef struct {
    ...
    u32 tcp_tx_buf_max_size;  ///< Maximum size of the TCP transfer (send) buffer. If it is 0, the size of the buffer
                              ///< is fixed to its initial value.
    u32 tcp_rx_buf_max_size;  ///< Maximum size of the TCP receive buffer. If it is 0, the size of the buffer is fixed
...
NX_INLINE Result socketInitializeDefault(void) { return socketInitialize(NULL); }
